...
# build reverse records from forward records
msg "building reverse from forward"
reverserecords="reverse_records.txt"
#cleaner $reverserecords
awk -F" " '{print $1 "_-_" $5}' "${forwardrecords}" > "${forwardrecords}.tmp" && mv "${forwardrecords}.tmp" "${forwardrecords}"
for frecord in `cat $forwardrecords`; do
    fqdn=${frecord%_-_*}
    ip=${frecord#*_-_}
    IFS=.
    read ip1 ip2 ip3 ip4 <<< "${ip}"
    unset IFS
    ptrfqdn="$ip4.$ip3.$ip2.$ip1.in-addr.arpa."
    echo "${ptrfqdn} 3600 IN PTR ${fqdn}" >> ${reverserecords}
done
if [[ ! -f "$reverserecords" ]] ; then msg "can't build reverse records (${reverserecords})" error; exit; fi
# build reverse zones
msg "building reverse zones"
for rzone in `cat $reversefile`; do
    echo "@ IN SOA ns.mrddi.org. hostmaster.mrddi.org. ( 1 900 600 86400 3600 )" > ${rzone}
    echo "@ NS ns.mrddi.org." >> ${rzone}
    grep ${rzone} ${reverserecords} >> ${rzone}
    zrows=$(cat $rzone | wc -l | awk '{$1=$1};1')
    if [ $zrows -lt 3 ]; then
      msg "no reverse records for ${rzone} found" debug;
      cleaner "${rzone}"
    else
      msg "reverse records collected for ${rzone}";
     fi
done
# orphaned records
orphan="orphaned_records.txt"
egrep -v -f ${reversefile} ${reverserecords} > ${orphan}
orows=$(cat $orphan | wc -l | awk '{$1=$1};1')
if [ $orows -lt 1 ]; then
  msg "no orphaned records found" debug;
  cleaner "${orphan}"
else
  msg "${orows} orphaned records in ${orphan} found";
 fi
# clean-up
msg "cleaning up"
cleaner "${forwardrecords}"
